...
// 더블버퍼 + 전송 전담 스레드 상태
static volatile bool     frame_pending   = false; // commit 된 프레임이 전송 대기/진행 중
static volatile uint8_t  draw_index      = 0;      // 앱이 그리는 백버퍼
static volatile uint8_t  send_index      = 0;      // 스레드가 전송 중인 프런트버퍼

static volatile uint32_t fps_pre_time;
static volatile uint32_t fps_time;
static volatile uint32_t fps_count = 0;

static volatile int32_t  draw_fps        = 10;
...
static uint16_t __attribute__((aligned(64), section(".ext_ram.bss"))) frame_buffer[2][LCD_WIDTH * LCD_HEIGHT];
static uint16_t __attribute__((aligned(64))) *p_draw_frame_buf = NULL;
...
K_SEM_DEFINE(lcd_sem, 0, 1); /* 초기 카운트 0, 최대 카운트 1의 세마포어 */

K_THREAD_STACK_DEFINE(lcd_thread_stack, _HW_DEF_RTOS_THREAD_MEM_LCD);
...
static void lcdThread(void *arg1, void *arg2, void *arg3);
...
bool lcdInit(void)
{
  ...
  is_init = st7306Init();
  st7306InitDriver(&lcd);


  if (is_init == true)
    logPrintf("[OK] st7789Init()\n");
  else
    logPrintf("[E_] st7789Init()\n");

  draw_index       = 0;
  send_index       = 0;
  frame_pending    = false;
  p_draw_frame_buf = frame_buffer[draw_index];

  // 전송 전담 스레드 생성. 아래 lcdLogoOn() 이 lcdUpdateDraw() 를 통해
  // 이 스레드 경유로 전송하므로 로고 출력 전에 생성해야 한다.
  k_thread_create(&lcd_thread_data, lcd_thread_stack,
                  K_THREAD_STACK_SIZEOF(lcd_thread_stack),
                  lcdThread, NULL, NULL, NULL,
                  _HW_DEF_RTOS_THREAD_PRI_LCD, 0, K_NO_WAIT);
...
static void lcdThread(void *arg1, void *arg2, void *arg3)
{
  ...
  while (1)
  {    
    k_sem_take(&lcd_sem, K_FOREVER);

    draw_pre_time = millis();

    lcd.setWindow(0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1);
    lcd.sendBuffer((uint8_t *)frame_buffer[send_index], LCD_WIDTH * LCD_HEIGHT, 100);
...
bool lcdDrawAvailable(void)
{
  ...
  if (draw_fps > 0)
  {
    if (!frame_pending && millis() - draw_pre_time >= (1000 / draw_fps))
    {
      ret = true;
    }
  }
...
// commit: 방금 그린 백버퍼를 전송 대상으로 넘기고 백버퍼를 스왑한 뒤
// 전송 스레드를 깨운다. 전송 자체는 스레드가 비동기로 처리하므로 즉시 리턴한다.
bool lcdRequestDraw(void)
{
  ...
  if (frame_pending == true)
  {
    return false;
  }

  send_index       = draw_index;
...
{
  return (uint16_t *)frame_buffer[draw_index];
}
...
image_t lcdCreateImage(lvgl_img_t *p_lvgl, int16_t x, int16_t y, int16_t w, int16_t h)
{
  ...
  if (w > 0)
    ...
    ret.w = p_lvgl->header.w;

  if (h > 0)
    ret.h = h;
  else
    ret.h = p_lvgl->header.h;
