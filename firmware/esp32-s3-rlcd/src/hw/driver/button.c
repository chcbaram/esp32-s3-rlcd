...
{
  {GPIO_DT_SPEC_GET(DT_NODELABEL(button1), gpios), _DEF_HIGH},
  {GPIO_DT_SPEC_GET(DT_NODELABEL(button0), gpios), _DEF_HIGH},
};
...
void cliButton(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "info"))
  {
    for (int i=0; i<BUTTON_MAX_CH; i++)
    {
      cliPrintf("button %d : %d, on_state %d\n", i, gpio_pin_get_dt(&button_pin[i].h_dt), button_pin[i].on_state);
    }
    ret = true;
  }
