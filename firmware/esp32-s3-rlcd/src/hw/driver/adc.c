...
{
  {ADC_DT_SPEC_GET(DT_NODELABEL(vbatt)), NAME_DEF(BAT_ADC)},
};
...
float adcConvVoltage(uint8_t ch, uint32_t adc_value)
{
  ...
  float       ret     = 0;
  float       gain    = 1.f/4.f;
  const float adc_max = 4095.f; // 12비트 ADC 기준 (0 ~ 4095)


  v_ref  = (float)adc_ref_internal(adc_tbl[ch].h_dt.dev) / 1000.f;
  v_ref += 0.02f; // 보정 

  switch (ch)
  {
    case BAT_ADC:
      ret  = ((float)adc_value / adc_max) * v_ref * ((200.f + 100.f) / 100.f);
      ret /= gain;
      break;

    default:
      ret = ((float)adc_value * v_ref) / adc_max;
      break;
  }
...
void cliAdc(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "info") == true)
  {
    ...
    cliPrintf("adc res  : %d\n", adcGetRes(0));
    cliPrintf("adc vref : %d\n", adc_ref_internal(adc_tbl[0].h_dt.dev));
